...
# ========== LOGGED IN AREA ==========
    ...
    # ========== CHANGE PIN — ✅ FULLY FIXED ==========
        ...
        if st.button("🔄 Update PIN", type="primary"):
            ...
            valid_current = account.verify_pin(current_pin.strip())
            if not valid_current:
                st.error("❌ Current PIN is incorrect")
            elif not new_pin_clean.isdigit() or len(new_pin_clean) != 4:
                st.error("❌ New PIN must be exactly 4 digits")
            elif new_pin_clean != confirm_clean:
                st.error("❌ PINs do not match")
            elif current_pin.strip() == new_pin_clean:
                st.warning("⚠️ New PIN must be different")
            else:
    ...
    # ========== ACCOUNT ANALYSIS — ✅ FULL WORKING FEATURE ==========
